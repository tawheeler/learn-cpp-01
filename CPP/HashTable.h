...
protected:
	HashTableNode<T>* LookupNode(std::string const &key)
	{
		...
		std::list< HashTableNode<T> >::iterator iter;
		iter = m_hash_table[index].begin();

		while(iter != m_hash_table[index].end())
		{
			if(iter->m_key == key)
			{
				return_value = &(*iter);
...
public:
    ...
    std::list< std::string > GetKeys() {
        ...
        for ( int i = 1; i < GetTableSize(); i ++ ) {
            ...
            if(m_hash_table[i].size() == 0)
			    continue;

            for ( iter = m_hash_table[i].begin(); iter != m_hash_table[i].end(); ++iter ) {
                retval.push_back(iter->m_key);
		    }
        }
